...
def build_report(before_text,after_text):
    ...
    return {"protected_tokens":token,"anchored_facts_legacy":anchor,"fidelity_relation_graph":g,"semantic_sentinels":sem,"conditions_exceptions":cond,"protected_structures":struct,"locale_register_drift":loc,"mechanical_lint":lint}

def main():
    ap=argparse.ArgumentParser(); ap.add_argument('before'); ap.add_argument('after'); ap.add_argument('--json',action='store_true'); args=ap.parse_args(); b=Path(args.before).read_text(encoding='utf-8'); a=Path(args.after).read_text(encoding='utf-8'); report=build_report(b,a); failures={k:v for k,v in report.items() if v}
