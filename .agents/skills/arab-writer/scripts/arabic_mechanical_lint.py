...
def lint(text: str):
    ...
    for code, pattern, msg in CHECKS:
        for m in pattern.finditer(text):
            ...
            findings.append({"code": code,"message": msg,"line": line,"column": col,"match": m.group(0)[:80]})
...
def main() -> int:
    ...
    findings = lint(Path(args.file).read_text(encoding="utf-8"))
    if args.json: print(json.dumps(findings, ensure_ascii=False, indent=2))
    elif not findings: print("OK: no heuristic lint findings."); return 0
    else:
        for f in findings: print(f'{args.file}:{f["line"]}:{f["column"]} [{f["code"]}] {f["message"]}: {f["match"]!r}')
